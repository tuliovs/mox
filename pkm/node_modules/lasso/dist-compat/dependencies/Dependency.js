...
function doCalculateFingerprint(dependency, lassoContext) {
    return new Promise((resolve, reject) => {
        ...
        var fingerprintStream = util.createFingerprintStream().on('error', reject).on('fingerprint', function (fingerprint) {
        ...
        input.on('error', function (e) {
            ...
            reject(e);
        }).pipe(cachingReadStream).pipe(fingerprintStream);
    });
};
...
Dependency.prototype = {
    ...
    init(lassoContext) {
        ...
        return _asyncToGenerator(function* () {
            _this._context = lassoContext;

            if (_this._resolvedInit) {
                return;
            }

            yield _this.doInit(lassoContext);
            _this._resolvedInit = true;
        })();
    },
    ...
    getPackageManifest(lassoContext) {
        var _this2 = this;

        return _asyncToGenerator(function* () {
            if (!_this2.isPackageDependency()) {
                throw new Error('getPackageManifest() failed. Dependency is not a package: ' + _this2.toString());
            }

            let manifest;

            if (typeof _this2.loadPackageManifest === 'function') {
                let packageManifestResult;
                packageManifestResult = yield _this2.loadPackageManifest(lassoContext);

                if (!packageManifestResult) {
                    return null;
                }

                var dependencyRegistry = _this2.__dependencyRegistry;
                var LassoManifest = require('../LassoManifest');

                if (typeof packageManifestResult === 'string') {
                    var manifestPath = packageManifestResult;
                    var from = _this2.getParentManifestDir();

                    try {
                        manifest = _this2.createPackageManifest(manifestLoader.load(manifestPath, from));
                    } catch (e) {
                        var err;
                        if (e.fileNotFound) {
                            err = new Error('Lasso manifest not found for path "' + manifestPath + '" (searching from "' + from + '"). Dependency: ' + _this2.toString());
                        } else {
                            err = new Error('Unable to load lasso manifest for path "' + manifestPath + '". Dependency: ' + _this2.toString() + '. Exception: ' + (e.stack || e));
                        }
                    ...
                    manifest = new LassoManifest({
                        ...
                        dirname: _this2.getParentManifestDir(),
                        filename: _this2.getParentManifestPath()
                    });
                } else {
                    manifest = packageManifestResult;
                }
                ...
                return manifest;
            } else if (typeof _this2.getDependencies === 'function') {
                let dependencies = yield _this2.getDependencies(lassoContext);
                if (dependencies) {
                    manifest = _this2.createPackageManifest(dependencies);
                }

                return manifest;
            } else {
                throw new Error('getPackageManifest() failed. "getDependencies" or "loadPackageManifest" expected: ' + _this2.toString());
            }
        })();
    },
    ...
    doCalculateKey(lassoContext) {
        if (this.isPackageDependency()) {
            ...
            if (lassoContext.cache) {
                return this.getLastModified(lassoContext).then(lastModified => {
                    if (!lastModified) {
    ...
    calculateKeyFromProps: function () {
        var key = this._getKeyPropertyNames().map(function (k) {
            return k + '=' + this[k];
        }, this).join('|');
    ...
    getLastModified(lassoContext) {
        ...
        return _asyncToGenerator(function* () {
            if (_this3._lastModifiedValue) {
                return _this3._lastModifiedValue;
            }

            const getLastModified = function (lastModified) {
                return lastModified == null || lastModified < 0 ? 0 : lastModified;
            };

            const lastModified = _this3._lastModifiedValue = getLastModified((yield _this3.doGetLastModified(lassoContext)));

            return lastModified;
        })();
    },

    doGetLastModified(lassoContext, callback) {
        var _this4 = this;

        return _asyncToGenerator(function* () {
            const sourceFile = _this4.getSourceFile();
            if (sourceFile) {
                return _this4.getFileLastModified(sourceFile);
            } else {
                return 0;
            }
        })();
    ...
    inspect: function () {
        ...
        this._getKeyPropertyNames().forEach(k => {
            inspected[k] = this[k];
        });
