 ...
 */
let writeFile = (() => {
    var _ref = _asyncToGenerator(function* (inStream, outputFile, calculateFingerprint, fingerprintLength) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (calculateFingerprint) {
                ...
                }).on('error', handleError).pipe(outStream);

                inStream.on('error', handleError).pipe(fingerprintStream);
            } else {
                ...
                inStream.on('error', handleError).pipe(outStream).on('close', function () {
                    if (done) {
                        return;
                    }
    ...
    return function writeFile(_x, _x2, _x3, _x4) {
...
module.exports = function fileWriter(fileWriterConfig, lassoConfig) {
    ...
    function getOutputFileForBundle(bundle) {
        ...
        return getOutputFile(relativePath, bundle.getName(), targetExt, bundle.getSlot());
    }

    function getOutputFileFromFileName(path, relativePath) {
        const filename = nodePath.basename(path);

        return getOutputFile(relativePath, filename);
    }
    ...
    return {
        ...
        writeBundle(reader, lassoContext) {
            return _asyncToGenerator(function* () {
                return new Promise(function (resolve, reject) {
                    ...
                    writeFile(input, outputFile, calculateFingerprint, fingerprintLength).then(function (result) {
        ...
        writeResource(reader, lassoContext) {
            return _asyncToGenerator(function* () {
                let done = false;
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    writeFile(input, outputFileForResource, calculateFingerprint, fingerprintLength).then(function (result) {
        ...
        writeResourceBuffer(buff, lassoContext) {
            return _asyncToGenerator(function* () {
                let done = false;
